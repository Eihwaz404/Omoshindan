<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold leading-tight text-slate-100">{{ __('Editar área') }}</h2>
            <p class="mt-1 text-sm text-slate-400">{{ $area->name }} · {{ $area->slug }}</p>
        </div>
    </x-slot>
        ...
        <div class="w-full px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
            ...
            <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-lg shadow-slate-950/40 sm:p-8">
                @include('support.areas.partials.form', [
                    'action' => route('support.areas.update', $area),
                    'method' => 'PUT',
